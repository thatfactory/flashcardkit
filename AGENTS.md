...
FlashcardKit is a UI-agnostic domain package for deterministic prompt-and-answer recall activities. Read [README.md](README.md) and the DocC catalog before changing public behavior.

Keep presentation, clocks, persistence, media resolution, vocabulary acquisition, and spaced-repetition policy outside the package. Favor seeded deterministic behavior and persistence-friendly `Codable`, `Sendable` values.
...
|---|---|
| Package sources | `Sources/FlashcardKit/` |
| DocC catalog | `Sources/FlashcardKit/FlashcardKit.docc/` |
| Unit tests | `Tests/FlashcardKitTests/` |

## Stack

- Swift 6.4 and Xcode 27.
- anyAppleOS 26 and later: iOS, macOS, tvOS, watchOS, and visionOS.
